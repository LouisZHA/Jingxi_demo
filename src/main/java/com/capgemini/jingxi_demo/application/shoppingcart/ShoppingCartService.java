...
public class ShoppingCartService {
    ...
    // 根据用户ID，返回全部购物车列表
    public List<ShoppingCartEntity> showShoppingCart(BigInteger userId){
        return shoppingCartRepository.GetShoppingCartByUserId(userId);
    }

    // 根据用户ID，清空用户的购物车
    public void cleanShoppingCart(BigInteger userId){
        shoppingCartRepository.deleteByUserId(userId);
        System.out.println("根据用户ID，清空用户的购物车");
    }

    // 根据用户ID和商品ID,删除购物车中的一种商品
    public void deleteProduct(BigInteger userid, BigInteger productid){
    ...
    public void addtoShoppingCart(ShoppingCartAddDTO shoppingCartAddDTO){
        ...
        BigInteger Product_id = shoppingCartAddDTO.getProduct_id();
        BigInteger User_id = shoppingCartAddDTO.getUser_id();
        ...
        if (productEntity.isPresent()){
            if(list != null && !list.isEmpty()){
                ...
                if (productEntity.get().getQuantity() - productEntity.get().getQuantity_locked() >= shoppingCartAddDTO.getQuantity()){
                    ...
                    productRepository.ModifyProductQuantityById(Product_id.longValue(), productEntity.get().getQuantity()-shoppingCartAddDTO.getQuantity());
                ...
                if (productEntity.get().getQuantity() - productEntity.get().getQuantity_locked() >= shoppingCartAddDTO.getQuantity()){
                    shoppingCartEntity.setProduct_name(productEntity.get().getName());
